...
function parseArgs(argv) {
  const out = { cmd: 'init', dir: process.cwd(), agents: null, all: false, force: false };
  for (const a of argv) {
...
const HELP = `cairn — a record-keeping structure any coding agent can follow.
  ...
  npx @doogi/cairn check            run wiki/check.sh
...
// ── rules payload ──────────────────────────────────────────────────────────
function rulesBody() {
  const raw = fs.readFileSync(path.join(TEMPLATE, 'CLAUDE.cairn.md'), 'utf8');
...
if (opts.cmd === 'check') check(opts.dir);
